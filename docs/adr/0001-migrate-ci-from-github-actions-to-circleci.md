...
1. **Create CircleCI account.** Sign up, install the GitHub App on the `acme-studio` org, add `markdownlint-config` as a project.
...
1. **Publish a canary package.** Manually publish `@acme-studio/markdownlint-config-canary` (a stripped-down copy or a placeholder) to npm using the existing manual bootstrap recipe in `CLAUDE.md`. This creates a package whose TP mapping can be freely edited without affecting `@acme-studio/markdownlint-config`.
2. **Configure CircleCI TP for the canary.** On `npmjs.com/package/@acme-studio/markdownlint-config-canary/access`, add a CircleCI Trusted Publisher mapping with the UUIDs from step 2.
...
1. **Switch the production package's TP mapping** from GitHub Actions to CircleCI on `npmjs.com/package/@acme-studio/markdownlint-config/access`. The mapping is single-active per finding (1), so this is the irreversible-during-cutover step. Rollback is "edit the mapping back to GitHub Actions" — quick, but not atomic.
...
2. **Tear down the canary package.** `npm deprecate @acme-studio/markdownlint-config-canary "Canary package — used to validate CircleCI TP migration, no longer needed"`.
...
- The custom shell-script driver is forkable into other `acme-studio` packages later if the migration succeeds.
