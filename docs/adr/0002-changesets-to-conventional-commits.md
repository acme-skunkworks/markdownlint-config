...
- **The PR title is the version signal.** The repo squash-merges, so the squash subject is the PR title; release-please parses it (`feat` → minor, `fix` → patch, `!`/`BREAKING CHANGE:` → major) to compute the bump. CI lints the title (`amannn/action-semantic-pull-request`).
...
- **A changelog-completeness CI gate** restores the coupling Changesets gave for free: a release-triggering PR title (`feat`/`fix`/breaking) must carry a dated `changelog/` entry.
