...
| `claude-code-review.yml` / `claude.yml` | ⏭️ skip     | Thin callers into shared-workflows' `reusable-claude.yml` / `reusable-claude-code-review.yml` (A-629). Need `CLAUDE_CODE_OAUTH_TOKEN`, and `act` may not resolve the cross-repo `acme-skunkworks/shared-workflows@<sha>` ref. The `act:*` scripts use `-W` to scope to specific workflows, so these aren't loaded by default. |
